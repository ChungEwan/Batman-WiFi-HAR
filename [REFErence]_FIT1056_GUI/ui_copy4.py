...
class ResultsFrame(ctk.CTkFrame):
    ...
    def display_chart(self, data, bg_color="transparent"):
        ...
        # Donut chart
        fig_donut, ax_donut = plt.subplots(figsize=(3.5, 3.5))
        fig_donut.patch.set_facecolor('#212121')
        ax_donut.set_facecolor('#212121')
        ...
        self.chart_canvas.get_tk_widget().grid(row=0, column=0, padx=10, pady=10, sticky="n")

        # Action image to right
        action_image_path = os.path.join(self.script_dir, "Logo", f"{self.output}.png")
        if os.path.exists(action_image_path):
            img = Image.open(action_image_path).resize((120, 180), Image.Resampling.LANCZOS)
            img_tk = ImageTk.PhotoImage(img)
            self.image_label = ctk.CTkLabel(self.top_container, image=img_tk, text="")
            self.image_label.image = img_tk
            self.image_label.grid(row=0, column=1, padx=10, pady=10, sticky="n")

        # Bar chart
        fig_bar, ax_bar = plt.subplots(figsize=(8, 3))
        fig_bar.patch.set_facecolor('#212121')
        ax_bar.set_facecolor('#212121')
        if top_3:
