...
class ResultsFrame(ctk.CTkFrame):
    ...
    def newOutput(self, data):
        """
        Update the ResultsFrame with a Plotly visualization.
        ...
        if plot_image:
            self.label.destroy()  # Remove the placeholder label
            self.label = ctk.CTkLabel(self, image=plot_image, text="")
            self.label.image = plot_image  # Keep a reference to avoid garbage collection
            self.label.grid(row=2, column=0, pady=10, sticky="n")
            print("Visualization successfully displayed.")
        else:
            print("Failed to generate or display the visualization.")

        
    def generate_plot(self, data):
        """
        Generate a Plotly figure with a circular progress bar and horizontal bars.
        :param data: Dictionary with activity names as keys and values as scores.
        :return: ImageTk.PhotoImage object of the Plotly figure.
        """
        if not data:
        ...
        # Create subplots: 1 row for circular progress, 1 row for horizontal bars
        fig = make_subplots(rows=2, cols=3, row_heights=[0.4, 0.6],
                            specs=[[{"type": "pie"}, None, None],
                                   [{"type": "xy", "colspan": 3}, None, None]])

        # Circular progress for the highest value
        if sorted_data:
            key, value = sorted_data[0]
            ...
            fig.add_trace(go.Pie(labels=['', ''],
                          ...
                          row=1, col=1)
            fig.update_layout(annotations=[dict(text=f"{key}<br>{percentage:.1f}%", x=0.1, y=0.85,
                                                font_size=14, showarrow=False)])

        # Horizontal bar chart for the next top 3 values
...
if __name__ == "__main__":
    ...
    results_frame.newOutput(example_data)
